...
public class Sample {
  public static void main(String[] args) throws Exception {
    ...
    try (BufferedInputStream bis =
            new BufferedInputStream(new FileInputStream(file.getPath() + "/sample.jpg"));
        BufferedOutputStream bos =
            new BufferedOutputStream(
                new FileOutputStream(
                    file.getPath() + "/sample_copy.jpg")); // ファイルを書き込むためのBufferedOutputStreamを作成
        ) { // ファイルを読み込むためのBufferedInputStreamを作成
      ...
      byte[] data = null;
      while ((data = bis.readNBytes(1024)).length != 0) { // 1024バイトずつ読み込むため、メモリエラーを回避しやすい
        bos.write(data); // 読み込んだデータを書き込む
      }
      bos.flush(); // バッファに残っているデータをすべて書き込む
    }
  }
}
